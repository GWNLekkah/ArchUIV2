...
// Render form for selecting the embedding name
function EmbeddingNameForm({ embeddingName, setEmbeddingName }) {
  function onChange(key, value) {
    setEmbeddingName(value);
  }
...
// Render form for selecting the embedding name
function DataQueryForm({ dataQuery, setDataQuery }) {
  function onChange(key, value) {
    setDataQuery(value);
  }
...
// Render form for selecting the embedding type
function EmbeddingTypeForm({
  currentType,
  setCurrentType,
  embeddingTypeOptions,
}) {
  return (
    <div className="text-white flex items-center space-x-4 mt-3 justify-between">
...
export default function Embeddings() {
  ...
  function createEmbedding() {
    let token =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6ImFyamFuIiwiZXhwIjoxNjk2NjcxMDI1fQ.xFVyu4eAnd6F-aUihu4daGCjMR0STbY5093hvGG_cbM";
    let body = {
      name: embeddingName,
      config: {
    ...
    body["config"]["params"][currentType + ".0"] = config[String(currentType)];
    fetch("https://localhost:8000/embeddings", {
      method: "POST",
      headers: {
  ...
  function uploadFile() {
    ...
    fetch("https://localhost:8000/embeddings/" + selectedEmbedding + "/file", {
      method: "POST",
      headers: {
        Authorization: "bearer " + token,
      },
              ...
              >
                <Dialog.Panel className=" w-fill transform overflow-hidden rounded-2xl bg-slate-700 p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-4">
                    ...
                    {EmbeddingNameForm({ embeddingName, setEmbeddingName })}
                    {DataQueryForm({ dataQuery, setDataQuery })}
                    <EmbeddingTypeForm
                      currentType={currentType}
                      setCurrentType={setCurrentType}
